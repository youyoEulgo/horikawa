...
use horikawa_core::Player;
use horikawa_ctl::ProcessorSettings;
use horikawa_protocol::{AppCommand, StateUpdate};

use crate::discord::DiscordPresence;
use crate::media_controls::{MediaControlCommand, MediaControlsHandler};

/// Converts a file path to a file:// URL for SMTC/MPRIS album art.
#[cfg(any(
    target_os = "windows",
    target_os = "macos",
    all(unix, not(target_os = "macos"))
...
fn copy_to_temp_and_get_url(source: &std::path::Path) -> Option<String> {
    ...
    let horikawa_temp = temp_dir.join("horikawa");

    tracing::debug!("Attempting to copy album art from: {:?}", source);

    if !horikawa_temp.exists() {
        if let Err(e) = std::fs::create_dir_all(&horikawa_temp) {
            tracing::warn!("Failed to create temp dir {:?}: {}", horikawa_temp, e);
            return None;
        }
    }

    let ext = source.extension().and_then(|e| e.to_str()).unwrap_or("jpg");
    let dest_path = horikawa_temp.join(format!("cover.{}", ext));
    ...
    match std::fs::copy(source, &dest_path) {
        ...
        Err(e) => {
            tracing::warn!(
                "Failed to copy album art from {:?} to {:?}: {}",
                source,
                dest_path,
                e
            );
        ...
        *force_update = false;
        ...
        if let Some(ref track_path) = current_track {
            ...
            let cover_url = cover_url.filter(|_| {
                let temp_path = std::env::temp_dir().join("horikawa");
                temp_path
                    .read_dir()
                    .map(|mut entries| {
                        entries.any(|e| {
...
/// Stub for platforms without media controls (BSD, etc.).
#[cfg(not(any(
    target_os = "windows",
    target_os = "macos",
    all(unix, not(target_os = "macos"))
...
fn update_discord(
    ...
    if *last_discord_track != current_track {
        if let Some(ref track_path) = current_track {
            ...
            discord.update(title.as_deref(), artist.as_deref(), album.as_deref());
        }
...
pub fn run_integrations(
    player: Arc<Player>,
    command_sender: horikawa_ctl::CommandSender,
