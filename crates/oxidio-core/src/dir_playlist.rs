...
/// Serializable structure stored as a `.oxidio` JSON file.
///
...
impl DirectoryPlaylist {
    ...
    /// Saves this directory playlist to a `.oxidio` JSON file.
    ///
    /// The file is written to the standard playlist directory
    /// (`~/.local/share/oxidio/playlists/<name>.oxidio`).
    pub fn save(&self) -> Result<PathBuf, DirPlaylistError> {
        ...
        let path = dir.join(format!("{}.oxidio", self.name));
    ...
    /// Loads a directory playlist from a `.oxidio` JSON file.
    pub fn load(name: &str) -> Result<Self, DirPlaylistError> {
        ...
        let path = dir.join(format!("{}.oxidio", name));
    ...
    pub fn list() -> Result<Vec<String>, DirPlaylistError> {
        ...
        for entry in fs::read_dir(&dir)? {
            let entry = entry?;
            let path = entry.path();
            if path.extension().and_then(|e| e.to_str()) == Some("oxidio") {
    ...
    ///
    /// Uses `~/.local/share/oxidio/playlists` on Linux/macOS,
    /// or `Music/Oxidio` on Windows.
