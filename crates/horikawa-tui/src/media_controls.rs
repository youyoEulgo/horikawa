    ...
    impl MediaControlsHandler {
        ...
        fn create_hidden_window() -> Option<HWND> {
            unsafe {
                ...
                let app_id = HSTRING::from("Horikawa.MusicPlayer");
                let _ = SetCurrentProcessExplicitAppUserModelID(&app_id);

                // Define window class name
                let class_name: Vec<u16> = "HorikawaSMTC\0".encode_utf16().collect();
                ...
                let window_name: Vec<u16> = "Horikawa Music Player\0".encode_utf16().collect();
